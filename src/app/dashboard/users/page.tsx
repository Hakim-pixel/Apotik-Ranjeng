...
import { Users, Plus, X, CheckCircle, AlertCircle, Shield } from "lucide-react";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-zinc-900 dark:text-white">Manajemen User</h1>
      ...
      <div className="bg-white dark:bg-zinc-900 rounded-2xl shadow-sm border border-zinc-200 dark:border-zinc-800 overflow-hidden">
        {loading ? (
          ...
          <div className="divide-y divide-zinc-100 dark:divide-zinc-800">
            {users.map(u => (
              <div key={u.id} className="flex items-center gap-4 px-5 py-4 hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors">
                <div className="h-10 w-10 rounded-xl bg-gradient-to-br from-emerald-400 to-teal-600 flex items-center justify-center text-white font-bold text-sm flex-shrink-0">
                  {u.name.charAt(0).toUpperCase()}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="font-semibold text-sm text-zinc-900 dark:text-white truncate">{u.name}</p>
                  <p className="text-xs text-zinc-500 truncate">{u.email}</p>
                </div>
                <span className={`px-2.5 py-1 rounded-full text-xs font-semibold flex-shrink-0 ${ROLE_CONFIG[u.role]?.color}`}>
                  {ROLE_CONFIG[u.role]?.label || u.role}
                </span>
                <p className="text-xs text-zinc-400 hidden md:block whitespace-nowrap">
        ...
        <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-zinc-900 rounded-2xl w-full max-w-md shadow-2xl">
            <div className="flex items-center justify-between p-6 border-b border-zinc-200 dark:border-zinc-800">
              <h2 className="text-lg font-bold text-zinc-900 dark:text-white">Tambah User Baru</h2>
            ...
            <div className="p-6 space-y-4">
              {[
                ...
                { label: "Email *", key: "email", type: "email", placeholder: "budi@apotik.com" },
